-- Only add missing parts (since branch_id column already exists)

-- Step 1: Add missing columns to messages table (only if they don't exist)
DO $$
BEGIN
    ...
    -- Add branch_point column if missing
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
