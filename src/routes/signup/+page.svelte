...
<div class="min-h-screen flex justify-center px-4 pt-12 relative z-20" style="background: var(--bg-primary);">
  <div class="p-6 max-w-sm w-full space-y-4">
    ...
    <div class="space-y-3">
      ...
      />
      <button 
        class="w-full border px-3 py-2 bg-blue-600 hover:bg-blue-700 border-blue-600 text-white rounded transition-colors disabled:opacity-50" 
        on:click={submit}
        disabled={loading}
      >
      ...
      <a 
        href="/login"
        class="block w-full text-sm underline text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 text-center"
      >
    ...
    {#if msg}
      <p class="text-sm {msg.includes('Check your email') ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'} text-center">
        {msg}
      </p>
    {/if}
  </div>
