...
{#if showModal}
  <!-- Modal Overlay -->
  <div class="fixed inset-0 backdrop-blur-sm z-[99999] flex items-center justify-center" style="padding: 1rem;">
    <!-- Modal Content -->
            ...
            {#if factValue.trim()}
              <FeatureGate {user} feature="say-less" requiredTier={1} let:hasAccess>
                <SayLessButton
                  ...
                  disabled={!factValue.trim() || !hasAccess}
                  size="sm"
                />
              </FeatureGate>
            {/if}
