      ...
      <div class="flex items-center justify-between gap-2 p-1 rounded hover:bg-zinc-50 {current?.id === project.id ? 'bg-blue-500 text-white' : ''}">
        ...
        <div class="shrink-0 flex items-center gap-1">
          <button 
            ...
            on:click|stopPropagation={() => openProjectSettings(project)}
          >
            <Settings size="18" />
          </button>
          <button class="text-red-500 hover:text-red-700 p-1" title="Delete" on:click|stopPropagation={() => deleteProject(project)}>
            <Trash size="18" />
          </button>
        </div>
      </div>
