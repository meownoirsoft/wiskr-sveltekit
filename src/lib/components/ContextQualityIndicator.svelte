<script>
  ...
  import { Info, TrendingUp, X, Pin, FileText, Users } from 'lucide-svelte';
  ...
  $: if (analysis && lastAnalysisScore !== score) {
    ...
    // If tooltip is currently open, refresh the analysis immediately
    if (wasTooltipOpen) {
      loadAnalysis();
    }
  }
...
<div class="quality-tooltip-container relative">
  <button
    class="flex items-center gap-2 px-2 py-1 rounded-lg transition-all hover:bg-opacity-10 hover:bg-white"
  ...
  <!-- Quality Improvement Tooltip -->
  {#if showTooltip}
    <div class="absolute top-full left-0 mt-2 w-80 bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-lg shadow-xl z-50" on:click|stopPropagation>
      <!-- Header -->
